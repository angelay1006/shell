...
int main() {
    ...
    WRITE(STDOUT_FILENO,
          "This program loops until it receives a SIGTSTP or other stop "
          "signal.\n\n");
    WRITE(STDOUT_FILENO,
          "It has a signal handler for SIGCONT that re-sends a SIGTSTP,\n"
          "so that it can never actually be resumed once stopped.\n\n");
    WRITE(STDOUT_FILENO,
          "Use it to test that processes are moved into the foreground "
          "correctly.\n\n");
